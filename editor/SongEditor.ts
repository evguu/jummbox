        ...
        // Options for mod channel
            ...
            for (let mod: number = 0; mod < Config.modCount; mod++) {
                ...
                if (instrument.modChannels[mod] < 0) {
                    ...
                    $("#modInstrumentText" + mod)[0].style.display = "none";
                    $("#modChannelText" + mod)[0].innerText = "Channel:";

                    //Hide setting select if channel is "none"
                    if (instrument.modChannels[mod] == -2) {
                        $("#modSettingText" + mod)[0].style.display = "none";
                        ((this._modSetBoxes[mod].parentElement) as HTMLDivElement).style.display = "none";
                    }
                    else {
                        $("#modSettingText" + mod)[0].style.display = "";
                    ...
                    $("#modInstrumentText" + mod)[0].style.display = (channel.instruments.length > 1) ? "" : "none";
                    $("#modChannelText" + mod)[0].innerText = (channel.instruments.length > 1) ? "Ch:" : "Channel:";
                    $("#modSettingText" + mod)[0].style.display = "";
                    ((this._modSetBoxes[mod].parentElement) as HTMLDivElement).style.display = "";
                ...
                if (filterType == "eq filter" || filterType == "note filter") {
                    $("#modFilterText" + mod)[0].style.display = "";
                    $("#modSettingText" + mod)[0].style.setProperty("margin-bottom", "2px");
                    ...
                    $("#modFilterText" + mod)[0].style.display = "none";
                    $("#modSettingText" + mod)[0].style.setProperty("margin-bottom", "0.9em");
    ...
    private _fileMenuHandler = (event: Event): void => {
        switch (this._fileMenu.value) {
            ...
            case "viewPlayer":
                ...
                break;
            case "copyEmbed":
                this._copyTextToClipboard(`<iframe width="384" height="60" style="border: none;" src="${new URL("player/index.html#song=" + this._doc.song.toBase64String(), location.href).href}"></iframe>`);
                break;
            case "songRecovery":
